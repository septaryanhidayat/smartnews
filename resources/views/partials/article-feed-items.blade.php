@foreach($articles as $index => $article)
    ...
    <article class="feed-item feed-item--big">
        <div class="feed-item__img-wrap feed-item__img-wrap--big">
            <a href="{{ route('article.show', $article->slug) }}" aria-label="{{ $article->title }}">
                <img class="feed-item__img feed-item__img--big" src="{{ $article->image_url }}" alt="{{ $article->title }}" width="600" height="340" loading="lazy" decoding="async" onerror="this.src='{{ asset('images/default-news.webp') }}'">
            </a>
    ...
    <article class="feed-item">
        <div class="feed-item__img-wrap">
            <a href="{{ route('article.show', $article->slug) }}" aria-label="{{ $article->title }}">
                <img class="feed-item__img" src="{{ $article->image_url }}" alt="{{ $article->title }}" width="240" height="160" loading="lazy" decoding="async" onerror="this.src='{{ asset('images/default-news.webp') }}'">
            </a>
            @if($article->media_type === 'video')
                <div class="media-badge media-badge--video">
                    <i class="fas fa-play"></i> <span>{{ $article->media_badge ?? 'Video' }}</span>
